...
#pragma warning restore CA2000
        ...
        public async Task<IBrowser> ConnectOverCDPAsync(string endpointURL, float? timeout = default, IEnumerable<KeyValuePair<string, string>> headers = default, string artifactsDir = default, bool? noDefaults = default)
        {
            ...
            {
                throw new PlaywrightSharpException("Connecting over CDP is only supported in Chromium and WebKit.");
            }
...
#pragma warning disable CA2000 // Ownership of the browser transfers to the caller.
                ...
                PlaywrightSharp.Chromium.ChromiumBrowser instance = new(crBrowser, resolvedArtifacts);
                ((IHasTracesDir)instance).TracesDir = resolvedArtifacts;
                if (noDefaults != true)
                {
                    foreach (IBrowserContext context in instance.Contexts)
                    {
                        if (context is PlaywrightSharp.Chromium.ChromiumBrowserContext defaultContext)
                        {
