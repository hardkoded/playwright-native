...
namespace PlaywrightSharp.Helpers
{
    ...
    internal static class WaitForSelectorName
    {
        ...
        internal static void Validate(string waitFor, string visibility)
        {
            if (visibility != null)
            {
                throw new PlaywrightSharpException("options.visibility is not supported, did you mean options.state?");
            }

            if (waitFor != null && !string.Equals(waitFor, "visible", StringComparison.OrdinalIgnoreCase))
            {
                throw new PlaywrightSharpException("options.waitFor is not supported, did you mean options.state?");
            }
        }
        ...
        private static PlaywrightSharpException InvalidState()
            => new PlaywrightSharpException("state: expected one of (attached|detached|visible|hidden)");
    }
}
