    ...
    internal static class FrameSelector
    {
            ...
            string selector)
        {
            ...
            if (IsAnyFrame(parts[0]))
            {
                ...
                {
                    throw new PlaywrightSharpException("Selector cannot be empty after frameLocator()");
                }
            ...
            if (IsPierce(parts[0]))
            {
                ...
                if (index >= parts.Count)
                {
                    throw new PlaywrightSharpException("Selector cannot end with entering frame");
                }
            ...
            string selector)
        {
            ...
            while (index < parts.Count)
            {
                ...
                {
                    throw new PlaywrightSharpException("\"any-frame\" is only allowed as the first selector token");
                }

                if (IsPierce(parts[index]))
                {
                    throw new PlaywrightSharpException("\"pierce-frames\" is only allowed as the first selector token");
                }

                if (!IsEnter(parts[index]))
                {
                    throw new PlaywrightSharpException("Selector cannot start with entering frame, select the iframe first");
                }

                index++;
                if (index >= parts.Count)
                {
                    throw new PlaywrightSharpException("Selector cannot end with entering frame, while parsing selector " + selector);
                }
            ...
            string chunk)
        {
            ...
            try
            {
                ...
                || PlaywrightSharpException.IsDestroyedContext(ex as PlaywrightSharpException)
                || (ex.Message != null && (
        ...
        private static void Validate(IReadOnlyList<string> parts, string selector)
        {
            if (parts.Count == 0)
            {
                throw new PlaywrightSharpException("Selector cannot be empty");
            }
