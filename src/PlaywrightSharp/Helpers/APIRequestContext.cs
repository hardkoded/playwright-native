...
namespace PlaywrightSharp.Helpers
{
    ...
    internal sealed partial class APIRequestContext : IAPIRequestContext
    {
            ...
            byte[] dataBytes = null)
        {
            ...
            {
                throw new PlaywrightSharpException("Protocol \"" + uri.Scheme + ":\" not supported");
            }
            ...
            if (json is Delegate)
            {
                throw new PlaywrightSharpException("Unexpected 'data' type given: " + json.GetType().Name);
            }
            ...
            if (redirectLimit < 0)
            {
                throw new PlaywrightSharpException("'maxRedirects' must be greater than or equal to '0'");
            }
            ...
            try
            {
                ...
                using (response)
                {
                    ...
                    catch (Exception ex) when (ex is not PlaywrightSharpException)
                    {
...
#pragma warning restore CA5359
            ...
            bool skipHost = false)
        {
            ...
            foreach (KeyValuePair<string, string> header in headers)
            {
                ...
                {
                    throw new PlaywrightSharpException("Invalid character in header content");
                }
            ...
            TlsCapture tlsCapture)
        {
            ...
            try
            {
                ...
                for (int attempt = 0; attempt <= maxRetries; attempt++)
                {
                    ...
                    {
                        request.Dispose();
                        throw new PlaywrightSharpException("Max redirect count exceeded", ex);
                    }
                ...
                throw new PlaywrightSharpException("APIRequest maxRetries exceeded: " + maxRetries + " " + url);
            }
            finally
            {
                linked?.Dispose();
            ...
            CancellationToken token)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        response.Dispose();
                        throw new PlaywrightSharpException("Max redirect count exceeded");
                    }
...
#pragma warning restore VSTHRD103
        ...
        private PlaywrightSharpException DisposedException(bool inFlight = false)
        {
            if (!string.IsNullOrEmpty(_closeReason))
            {
                return new PlaywrightSharpException(_closeReason);
            }

            if (inFlight)
            {
                return new PlaywrightSharpException("Request context disposed.");
            }

            return new PlaywrightSharpException(DriverMessages.BrowserOrContextClosedExceptionMessage);
        }
        ...
        private void EnsureNotDisposed()
        {
            ...
            if (!string.IsNullOrEmpty(_closeReason))
            {
                throw new PlaywrightSharpException(_closeReason);
            }

            throw new PlaywrightSharpException(DriverMessages.BrowserOrContextClosedExceptionMessage);
        }
        ...
        private async Task<string> ResolveUserAgentAsync()
        {
            ...
            if (_context != null)
            {
                foreach (IPage page in _context.Pages)
                {
                    ...
                    catch (Exception ex) when (ex is PlaywrightSharpException || ClosedTarget.IsClosed(ex))
                    {
