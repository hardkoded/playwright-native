...
namespace PlaywrightSharp.Helpers
{
    ...
    internal sealed class TracingHar
    {
        ...
        internal Task<IAsyncDisposable> StartAsync(string path, HarContentPolicy content = default, HarMode mode = default, string url = default, Regex urlRegex = default, string resourcesDir = default)
        {
            ...
            {
                throw new PlaywrightSharpException("resourcesDir option is not compatible with a .zip har file");
            }

            if (_started)
            {
                throw new PlaywrightSharpException("HAR recording has already been started");
            }
        ...
        internal async Task StopAsync()
        {
            if (!_started)
            {
                throw new PlaywrightSharpException("HAR recording has not been started");
            }
