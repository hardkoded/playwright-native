...
namespace PlaywrightSharp.Helpers
{
    ...
    internal static class ClockScript
    {
        ...
        internal static long ParseTime(long time)
        {
            ...
            {
                throw new PlaywrightSharpException("Invalid date: " + FormatNumber(time));
            }
        ...
        internal static long ParseTime(string time)
        {
            ...
            {
                throw new PlaywrightSharpException("Invalid date: (empty)");
            }

            if (!DateTime.TryParse(time, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, out DateTime parsed))
            {
                throw new PlaywrightSharpException("Invalid date: " + time);
            }
        ...
        internal static long ParseTicks(string ticks)
        {
            ...
            {
                throw new PlaywrightSharpException("Clock only understands numbers, 'mm:ss' and 'hh:mm:ss'");
            }
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                ...
                if (parsed >= 60)
                {
                    throw new PlaywrightSharpException("Invalid time " + ticks);
                }
        ...
        private static string LoadSource()
        {
            ...
            using Stream stream = assembly.GetManifestResourceStream("PlaywrightSharp.Helpers.clockSource.js")
                ?? throw new PlaywrightSharpException("Bundled Playwright clock source is missing.");
