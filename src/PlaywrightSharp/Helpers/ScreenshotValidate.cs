...
namespace PlaywrightSharp.Helpers
{
    ...
    internal static class ScreenshotValidate
    {
        ...
        internal static ScreenshotType ResolveType(string path, ScreenshotType type)
        {
            ...
            throw new PlaywrightSharpException("path: unsupported mime type \"" + (mime ?? "null") + "\"");
        }
        ...
        internal static void EnsureQuality(ScreenshotType type, int? quality)
        {
            ...
            {
                throw new PlaywrightSharpException("options.quality is unsupported for the " + format + " screenshots");
            }
        ...
        internal static void EnsureClip(Clip clip, bool fullPage, PageViewportSizeResult viewport)
        {
            ...
            if (clip.Width <= 0)
            {
                throw new PlaywrightSharpException("Expected options.clip.width to be greater than 0");
            }

            if (clip.Height <= 0)
            {
                throw new PlaywrightSharpException("Expected options.clip.height to be greater than 0");
            }
            ...
            if (x2 - x1 <= 0 || y2 - y1 <= 0)
            {
                throw new PlaywrightSharpException("Clipped area is either empty or outside the resulting image");
            }
        }
