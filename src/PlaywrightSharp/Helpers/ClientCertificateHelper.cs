    ...
    internal static class ClientCertificateHelper
    {
        ...
        internal static void Verify(IEnumerable<ClientCertificate> certificates)
        {
            ...
            foreach (ClientCertificate certificate in certificates)
            {
                ...
                {
                    throw new PlaywrightSharpException("clientCertificates.origin is required");
                }
                ...
                {
                    throw new PlaywrightSharpException(MissingMaterialMessage);
                }

                if (hasCert && !hasKey)
                {
                    throw new PlaywrightSharpException("cert is specified without key");
                }

                if (!hasCert && hasKey)
                {
                    throw new PlaywrightSharpException("key is specified without cert");
                }

                if (hasPfx && (hasCert || hasKey))
                {
                    throw new PlaywrightSharpException(PfxConflictMessage);
                }
            }
        }
        ...
        /// </param>
        /// <returns>A <see cref="PlaywrightSharpException"/>.</returns>
        internal static PlaywrightSharpException RewriteLoadException(Exception ex, bool forBrowser)
        {
            ...
            return new PlaywrightSharpException(rewritten, ex);
        }
...
#pragma warning restore SYSLIB0057
        ...
        private static void ThrowIfLegacyPfx(byte[] pfx)
        {
            ...
            {
                throw new PlaywrightSharpException(UnsupportedTlsCertificateMessage);
            }
        }
