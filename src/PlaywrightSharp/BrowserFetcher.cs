...
namespace PlaywrightSharp
{
    ...
    public sealed class BrowserFetcher : IBrowserFetcher
    {
        ...
        public async Task<InstalledBrowser> DownloadAsync(string buildId)
        {
            ...
            catch (Exception ex) when (ex is not OperationCanceledException && ex is not PlaywrightSharpException)
            {
                if (Directory.Exists(installDir))
                {
                    try
                ...
                throw new PlaywrightSharpException($"Failed to download {Browser} build {buildId}: {ex.Message}", ex);
            }
            finally
            {
