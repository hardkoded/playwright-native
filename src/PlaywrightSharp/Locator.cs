...
namespace PlaywrightSharp
{
    ...
    try {
            ...
            AbortSignal signal = default)
        {
            ...
            {
                throw new PlaywrightSharpException(ex.Message + "\nwaiting for " + ToString(), ex);
            }
            finally
            {
            ...
            ActionScroll scroll = default)
        {
            ...
            {
                throw new PlaywrightSharpException("Target locator must belong to the same page.");
            }
        ...
        private static Locator RequireLocator(ILocator other)
        {
            ...
            throw new ArgumentException("other must be a PlaywrightSharp locator.", nameof(other));
        }
        ...
        private static async Task ThrowIfHostIsNotFrameAsync(IElementHandle host)
        {
            ...
            throw new PlaywrightSharpException((html ?? string.Empty) + "\n<iframe> was expected");
        }
        ...
        private Locator RequireSameFrame(ILocator other)
        {
            ...
            {
                throw new PlaywrightSharpException("Locators must belong to the same frame.");
            }
        ...
        private async Task<IElementHandle> WaitForHandleAsync(float? timeout, string apiName, bool skipHandlers = false)
        {
            if (TryAriaRefBody(out string ariaRef))
            {
                ...
                if (found == null)
                {
                    throw new PlaywrightSharpException("No element matching aria-ref=" + ariaRef);
                }
        ...
        private async Task<IElementHandle> ResolveOneOrNullAsync()
        {
            ...
            if (all.Count > 1)
            {
                ...
                {
                    throw new PlaywrightSharpException("Pierce-frame mode matched elements from multiple frames");
                }
                ...
                throw new PlaywrightSharpException(strict);
            }
        ...
        private async Task<IReadOnlyList<IElementHandle>> ResolveAllAsync(IElementHandle root = null)
        {
            ...
            {
                throw new PlaywrightSharpException("Can't query n-th element");
            }
        ...
        private async Task<IReadOnlyList<IElementHandle>> QueryAnyFrameTreeAsync(IFrame frame, IElementHandle scope)
        {
            ...
            catch (Exception ex) when (IsFrameScopeTransient(ex) || PlaywrightSharpException.IsDestroyedContext(ex))
            {
        ...
        private Locator ApplyCommonFramePrefix(Locator inner)
        {
            ...
            {
                throw new PlaywrightSharpException("Can not capture the selector before diving into the frame. Only use * after the last frame has been selected");
            }
        ...
        private PlaywrightSharpException MultipleFramesException()
        {
