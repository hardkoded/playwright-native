...
#pragma warning disable CA2000 // Ownership of crBrowser transfers to ChromiumBrowser.
                PlaywrightSharp.Chromium.CRBrowser crBrowser = await PlaywrightSharp.Chromium.ChromiumBrowserType
                ...
                PlaywrightSharp.Chromium.ChromiumBrowser chromium = new PlaywrightSharp.Chromium.ChromiumBrowser(crBrowser, ResolveDownloadsPath(options), options.Logger)
                {
                    LaunchProxy = options.Proxy,
                };
                ((PlaywrightSharp.Helpers.IHasTracesDir)chromium).TracesDir = options.TracesDir;
                ((PlaywrightSharp.Helpers.IHasArtifactsDir)chromium).ArtifactsDir = options.ArtifactsDir;
                return chromium;
...
#pragma warning disable CA2000 // Ownership of ffBrowser transfers to FirefoxBrowser.
                PlaywrightSharp.Firefox.FFBrowser ffBrowser = await PlaywrightSharp.Firefox.FirefoxBrowserType
                ...
                return new PlaywrightSharp.Firefox.FirefoxBrowser(ffBrowser);
#pragma warning restore CA2000
            }
            catch (Exception ex)
            {
...
#pragma warning disable CA2000 // Ownership of wkBrowser transfers to the caller.
                ...
                ((PlaywrightSharp.Helpers.IHasTracesDir)wkBrowser).TracesDir = options.TracesDir;
                ((PlaywrightSharp.Helpers.IHasArtifactsDir)wkBrowser).ArtifactsDir = options.ArtifactsDir;
                return wkBrowser;
#pragma warning restore CA2000
            }
            catch (Exception ex)
...
#pragma warning disable CA2000 // Ownership of crBrowser transfers to ChromiumBrowser.
                ...
                PlaywrightSharp.Chromium.ChromiumBrowser instance = new(crBrowser, ResolveDownloadsPath(options))
                {
                    LaunchProxy = options.Proxy,
                };
                ((PlaywrightSharp.Helpers.IHasTracesDir)instance).TracesDir = options.TracesDir;
                ((PlaywrightSharp.Helpers.IHasArtifactsDir)instance).ArtifactsDir = options.ArtifactsDir;
#pragma warning restore CA2000
                IBrowserContext context = instance.PersistentContext();
                if (context is PlaywrightSharp.Chromium.ChromiumBrowserContext chromiumCerts)
                {
...
#pragma warning disable CA2000 // Ownership of ffBrowser transfers to FirefoxBrowser.
            ...
            PlaywrightSharp.Firefox.FirefoxBrowser instance = new(ffBrowser);
#pragma warning restore CA2000
            IBrowserContext context = instance.PersistentContext();
            await ApplyPersistentEmulationAsync(context, options).ConfigureAwait(false);
            return context;
...
#pragma warning restore CA2000
        ...
        private static async Task ApplyPersistentEmulationAsync(IBrowserContext context, BrowserTypeLaunchOptions options)
        {
            ...
            if (context is PlaywrightSharp.Chromium.ChromiumBrowserContext chromiumBase)
            {
                ...
                && !ClientCertificateHelper.HasAny(persistent.ClientCertificates))
            {
                ...
                if (context is PlaywrightSharp.Chromium.ChromiumBrowserContext chromiumWorkers)
                {
            ...
            if (context is PlaywrightSharp.Chromium.ChromiumBrowserContext chromium)
            {
        ...
        private static void ThrowIfPageArgument(IEnumerable<string> args)
        {
            ...
            foreach (string arg in args)
            {
                ...
                {
                    throw new PlaywrightSharpException("Arguments can not specify page to be opened");
                }
            }
        }
        ...
        private static async Task<string> ResolveExecutablePathAsync(SupportedBrowser browser, BrowserTypeLaunchOptions options)
        {
            ...
            if (options.Channel != BrowserChannel.Undefined)
            {
                ...
                {
                    throw new PlaywrightSharpException("Browser channel is only supported when launching Chromium.");
                }
