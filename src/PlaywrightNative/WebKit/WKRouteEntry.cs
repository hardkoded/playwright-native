...
{
    /// <summary>
    /// A registered WebKit route: glob, regex, or predicate plus handler identity
    /// so <c>UnrouteAsync</c> can remove the same registration.
    /// </summary>
    internal sealed class WKRouteEntry
    {
        private readonly object _remainingLock = new();
        private int? _remaining;

        /// <summary>
        /// Initializes a new instance of the <see cref="WKRouteEntry"/> class.
        /// </summary>
        ...
        /// </param>
        internal WKRouteEntry(
            string urlString,
            Regex urlRegex,
            Func<string, bool> urlFunc,
            Func<WKRoute, Task> handler,
        ...
        internal Func<WKRoute, Task> Handler { get; }
