...
{
    /// <summary>
    /// Official <c>scroll</c> option on <see cref="IPage.UncheckAsync"/>.
    /// </summary>
    [TestFixture]
    public class PageUncheckScrollTests : PageTestEx
    {
        [PlaywrightTest("page-check.spec.ts", "Uncheck Auto scrolls the target into view")]
        [Test]
        [Timeout(30_000)]
        public async Task AutoShouldScrollTheTargetIntoView()
        {
            ...
            await page.UncheckAsync("#btn").ConfigureAwait(false);
        ...
        [PlaywrightTest("page-check.spec.ts", "Uncheck scroll None does not scroll")]
        [Test]
        [Timeout(30_000)]
        public async Task ScrollNoneShouldNotScrollThePage()
        {
            ...
            try
            {
                await page.UncheckAsync("#btn", new() { Force = true, Scroll = ScrollMode.None }).ConfigureAwait(false);
            }
