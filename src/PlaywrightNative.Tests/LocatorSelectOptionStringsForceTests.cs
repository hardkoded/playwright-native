...
{
    /// <summary>
    /// Official <c>locator.selectOption(IEnumerable&lt;string&gt;, { force })</c>.
    /// </summary>
    [TestFixture]
    public class LocatorSelectOptionStringsForceTests : PageTestEx
    {
        private static IEnumerable<string> WaveValues()
            => new List<string> { "wave686" };
        ...
        public async Task ForceTrueShouldSelectAHiddenSelect()
        {
            ...
            await page.SetContentAsync("<select id='only' style='display:none'><option value='wave686'>a</option></select>").ConfigureAwait(false);

            await page.Locator("#only").SelectOptionAsync(WaveValues(), options: new() { Force = true }).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.getElementById('only').value").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("wave686"));
        }
        ...
        public async Task OmittedForceShouldTimeoutOnAHiddenSelect()
        {
            ...
            TimeoutException ex = Assert.ThrowsAsync<TimeoutException>(
                () => page.Locator("#only").SelectOptionAsync(WaveValues(), options: new() { Timeout = 200 }));
        ...
        public async Task ForceTrueShouldAcceptAVisibleUniqueSelector()
        {
            ...
            await using IBrowserContext context = await browser.NewContextAsync().ConfigureAwait(false);
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<select id='only'><option value='wave686'>a</option></select>").ConfigureAwait(false);
        ...
        public async Task FrameLocatorShouldHonorForce()
        {
            ...
            await frame.SetContentAsync("<select id='only' style='display:none'><option value='wave686'>a</option></select>").ConfigureAwait(false);

            await frame.Locator("#only").SelectOptionAsync(WaveValues(), options: new() { Force = true }).ConfigureAwait(false);
