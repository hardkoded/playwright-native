...
{
    /// <summary>
    /// Expect ToHaveScreenshot against golden bytes or a file.
    /// </summary>
    [TestFixture]
    public class ExpectScreenshotTests : PageTestEx
    {
        ...
        public async Task ToHaveScreenshotShouldMatchCapturedBytes()
        {
            ...
            await Assertions.Expect(page.Locator("#box")).ToHaveScreenshotAsync(expected).ConfigureAwait(false);
        }
        ...
        {
            string file = Path.Combine(Path.GetTempPath(), $"pwsharp-expect-shot-{Guid.NewGuid():N}.png");
            try
            {
                await using IBrowser browser = await BrowserLauncher.LaunchAsync().ConfigureAwait(false);
                await using IBrowserContext context = await browser.NewContextAsync().ConfigureAwait(false);
                ...
                await page.SetContentAsync(
                    "<style>html,body{margin:0}</style>" +
                    "<div id=\"box\" style=\"width:80px;height:50px;background:#00c\"></div>").ConfigureAwait(false);

                await page.Locator("#box").ScreenshotAsync(new() { Path = file }).ConfigureAwait(false);
                await Assertions.Expect(page.Locator("#box")).ToHaveScreenshotAsync(file).ConfigureAwait(false);
            }
            finally
            {
                if (File.Exists(file))
        ...
        public async Task ToHaveScreenshotShouldWaitUntilPixelsMatch()
        {
            ...
            byte[] expected = await page.Locator("#box").ScreenshotAsync().ConfigureAwait(false);
            await page.EvaluateAsync<object>("document.querySelector('#box').style.background = '#c00'").ConfigureAwait(false);

            Task assertTask = Assertions.Expect(page.Locator("#box")).ToHaveScreenshotAsync(expected, timeout: 5000);
            await Task.Delay(200).ConfigureAwait(false);
            await page.EvaluateAsync<object>("document.querySelector('#box').style.background = '#0a0'").ConfigureAwait(false);
        ...
        public async Task ToHaveScreenshotShouldTimeoutOnAMismatch()
        {
            ...
            TimeoutException ex = Assert.CatchAsync<TimeoutException>(
                () => Assertions.Expect(page.Locator("#box")).ToHaveScreenshotAsync(expected, timeout: 400));
        ...
        public async Task NotToHaveScreenshotShouldMatchADifferentImage()
        {
            ...
            byte[] green = await page.Locator("#box").ScreenshotAsync().ConfigureAwait(false);
            await page.EvaluateAsync<object>("document.querySelector('#box').style.background = '#c00'").ConfigureAwait(false);
            await Assertions.Expect(page.Locator("#box")).Not.ToHaveScreenshotAsync(green).ConfigureAwait(false);
        }
    }
}
