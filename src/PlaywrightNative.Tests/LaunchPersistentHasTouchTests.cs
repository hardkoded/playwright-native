...
{
    /// <summary>
    /// Official persistent-context <c>hasTouch</c> launch option.
    /// </summary>
    [TestFixture]
    public class LaunchPersistentHasTouchTests : PageTestEx
    {
        [PlaywrightTest("defaultbrowsercontext-1.spec.ts", "LaunchPersistentContextAsync HasTouch")]
        [Test]
        [Timeout(60_000)]
        public async Task LaunchPersistentContextAsyncShouldHonorHasTouch()
        {
            ...
            string userDataDir = Path.Combine(Path.GetTempPath(), "pwsharp-persist-touch-" + Guid.NewGuid().ToString("N"));
