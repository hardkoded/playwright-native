...
{
    /// <summary>
    /// Official persistent-context <c>recordHarPath</c> launch option.
    /// </summary>
    [TestFixture]
    public class LaunchPersistentRecordHarPathTests : PageTestEx
    {
        private static SimpleServer Server => TestServerSetup.Server;

        [PlaywrightTest("defaultbrowsercontext-1.spec.ts", "LaunchPersistentContextAsync RecordHarPath")]
        [Test]
        [Timeout(60_000)]
        public async Task LaunchPersistentContextAsyncShouldHonorRecordHarPath()
        {
            if (Server == null)
            {
            ...
            Server.Reset();
            Server.SetRoute("/har-persist.html", http =>
            {
                http.Response.ContentType = "text/html";
                return http.Response.WriteAsync("<html><body>wave-491-har</body></html>");
            });
            ...
            string userDataDir = Path.Combine(Path.GetTempPath(), "pwsharp-persist-har-" + Guid.NewGuid().ToString("N"));
            string harPath = Path.Combine(Path.GetTempPath(), "pwsharp-persist-har-" + Guid.NewGuid().ToString("N") + ".har");
            ...
            try
            {
                ...
                await page.GoToAsync(TestConstants.ServerUrl + "/har-persist.html").ConfigureAwait(false);
                ...
                foreach (JsonElement entry in entries.EnumerateArray())
                {
                    ...
                    if (url != null && url.Contains("/har-persist.html", StringComparison.Ordinal))
                    {
                        found = true;
                        break;
                    }
