...
{
    /// <summary>
    /// Official <c>page.uncheck({ strict })</c>.
    /// </summary>
    [TestFixture]
    public class UncheckStrictTests : PageTestEx
    {
        ...
        public async Task StrictTrueShouldThrowWhenTwoNodesMatch()
        {
            ...
            await page.SetContentAsync("<input type='checkbox' checked><input type='checkbox' checked>").ConfigureAwait(false);

            PlaywrightNativeException ex = Assert.CatchAsync<PlaywrightNativeException>(
                () => page.UncheckAsync("input", new() { Strict = true }));
        ...
        public async Task StrictTrueShouldAcceptAUniqueSelector()
        {
            ...
            await using IBrowserContext context = await browser.NewContextAsync().ConfigureAwait(false);
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<input id='only' type='checkbox' checked><input type='checkbox' checked>").ConfigureAwait(false);

            await page.UncheckAsync("#only", new() { Strict = true }).ConfigureAwait(false);
        ...
        public async Task StrictFalseShouldOverrideContextStrictSelectors()
        {
            ...
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<input id='first' type='checkbox' checked><input id='second' type='checkbox' checked>").ConfigureAwait(false);

            await page.UncheckAsync("input", new() { Strict = false }).ConfigureAwait(false);
        ...
        public async Task FrameShouldHonorStrict()
        {
            ...
            await frame.SetContentAsync("<input type='checkbox' checked><input type='checkbox' checked>").ConfigureAwait(false);

            PlaywrightNativeException ex = Assert.CatchAsync<PlaywrightNativeException>(
                () => frame.UncheckAsync("input", new() { Strict = true }));
