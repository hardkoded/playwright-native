...
{
    /// <summary>
    /// Official persistent-context <c>colorScheme</c> launch option.
    /// </summary>
    [TestFixture]
    public class LaunchPersistentColorSchemeTests : PageTestEx
    {
        [PlaywrightTest("defaultbrowsercontext-1.spec.ts", "LaunchPersistentContextAsync ColorScheme")]
        [Test]
        [Timeout(60_000)]
        public async Task LaunchPersistentContextAsyncShouldHonorColorScheme()
        {
            ...
            string userDataDir = Path.Combine(Path.GetTempPath(), "pwsharp-persist-cs-" + Guid.NewGuid().ToString("N"));
