...
{
    /// <summary>
    /// Official persistent-context <c>offline</c> launch option.
    /// </summary>
    [TestFixture]
    public class LaunchPersistentOfflineTests : PageTestEx
    {
        [PlaywrightTest("defaultbrowsercontext-1.spec.ts", "LaunchPersistentContextAsync Offline")]
        [Test]
        [Timeout(60_000)]
        public async Task LaunchPersistentContextAsyncShouldHonorOffline()
        {
            ...
            string userDataDir = Path.Combine(Path.GetTempPath(), "pwsharp-persist-offline-" + Guid.NewGuid().ToString("N"));
