...
{
    /// <summary>
    /// Official <c>scroll</c> option on <see cref="IPage.HoverAsync"/>.
    /// </summary>
    [TestFixture]
    public class PageHoverScrollTests : PageTestEx
    {
        [PlaywrightTest("page-click.spec.ts", "Hover Auto scrolls the target into view")]
        [Test]
        [Timeout(30_000)]
        public async Task AutoShouldScrollTheTargetIntoView()
        {
            ...
            await page.HoverAsync("#btn").ConfigureAwait(false);
        ...
        [PlaywrightTest("page-click.spec.ts", "Hover scroll None does not scroll")]
        [Test]
        [Timeout(30_000)]
        public async Task ScrollNoneShouldNotScrollThePage()
        {
            ...
            await page.HoverAsync("#btn", new() { Force = true, Scroll = ScrollMode.None }).ConfigureAwait(false);
            double after = await page.EvaluateAsync<double>("document.getElementById('box').scrollTop").ConfigureAwait(false);
