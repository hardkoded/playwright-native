...
{
    /// <summary>
    /// Official <c>page.selectOption(IEnumerable&lt;string&gt;, { force })</c>.
    /// </summary>
    [TestFixture]
    public class SelectOptionStringsForceTests : PageTestEx
    {
        private static IEnumerable<string> WaveValues()
            => new List<string> { "wave681" };
        ...
        public async Task ForceTrueShouldSelectAHiddenSelect()
        {
            ...
            await page.SetContentAsync("<select id='only' style='display:none'><option value='wave681'>a</option></select>").ConfigureAwait(false);

            await page.SelectOptionAsync("#only", WaveValues(), force: true).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.getElementById('only').value").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("wave681"));
        }
        ...
        public async Task OmittedForceShouldTimeoutOnAHiddenSelect()
        {
            ...
            TimeoutException ex = Assert.ThrowsAsync<TimeoutException>(
                () => page.SelectOptionAsync("#only", WaveValues(), timeout: 200));
        ...
        public async Task ForceTrueShouldAcceptAVisibleUniqueSelector()
        {
            ...
            await using IBrowserContext context = await browser.NewContextAsync().ConfigureAwait(false);
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<select id='only'><option value='wave681'>a</option></select>").ConfigureAwait(false);
        ...
        public async Task FrameShouldHonorForce()
        {
            ...
            await frame.SetContentAsync("<select id='only' style='display:none'><option value='wave681'>a</option></select>").ConfigureAwait(false);

            await frame.SelectOptionAsync("#only", WaveValues(), force: true).ConfigureAwait(false);
