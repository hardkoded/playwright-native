...
{
    /// <summary>
    /// Official <c>page.getAttribute({ strict })</c>.
    /// </summary>
    [TestFixture]
    public class GetAttributeStrictTests : PageTestEx
    {
        ...
        public async Task StrictTrueShouldAcceptAUniqueSelector()
        {
            ...
            await using IBrowserContext context = await browser.NewContextAsync().ConfigureAwait(false);
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<div id='only' data-x='wave654'></div><div data-x='b'></div>").ConfigureAwait(false);

            await page.GetAttributeAsync("#only", "data-x", new() { Strict = true }).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.getElementById('only').getAttribute('data-x')").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("wave654"));
        }
        ...
        public async Task StrictFalseShouldOverrideContextStrictSelectors()
        {
            ...
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<div id='first' data-x='wave654'></div><div data-x='b'></div>").ConfigureAwait(false);

            await page.GetAttributeAsync("div", "data-x", new() { Strict = false }).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.getElementById('first').getAttribute('data-x')").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("wave654"));
        }
