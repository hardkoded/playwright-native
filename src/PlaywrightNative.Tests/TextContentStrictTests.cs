...
{
    /// <summary>
    /// Official <c>page.textContent({ strict })</c>.
    /// </summary>
    [TestFixture]
    public class TextContentStrictTests : PageTestEx
    {
        ...
        public async Task StrictTrueShouldAcceptAUniqueSelector()
        {
            ...
            await using IBrowserContext context = await browser.NewContextAsync().ConfigureAwait(false);
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<span id='only'>wave657</span><span>two</span>").ConfigureAwait(false);

            await page.TextContentAsync("#only", new() { Strict = true }).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.getElementById('only').textContent").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("wave657"));
        }
        ...
        public async Task StrictFalseShouldOverrideContextStrictSelectors()
        {
            ...
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<span id='first'>wave657</span><span>two</span>").ConfigureAwait(false);

            await page.TextContentAsync("span", new() { Strict = false }).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.getElementById('first').textContent").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("wave657"));
        }
        ...
        public async Task FrameShouldHonorStrict()
        {
            ...
            PlaywrightNativeException ex = Assert.CatchAsync<PlaywrightNativeException>(
                () => frame.TextContentAsync("span", new() { Strict = true }));
