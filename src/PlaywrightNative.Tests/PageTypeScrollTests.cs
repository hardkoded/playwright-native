...
{
    /// <summary>
    /// Official <c>scroll</c> option on <see cref="IPage.TypeAsync"/>.
    /// </summary>
    [TestFixture]
    public class PageTypeScrollTests : PageTestEx
    {
        [PlaywrightTest("elementhandle-type.spec.ts", "Type Auto scrolls the target into view")]
        [Test]
        [Timeout(30_000)]
        public async Task AutoShouldScrollTheTargetIntoView()
        {
            ...
            await page.TypeAsync("#btn", "wave428").ConfigureAwait(false);
            ...
            Assert.That(value, Is.EqualTo("wave428"));
        }

        [PlaywrightTest("elementhandle-type.spec.ts", "Type scroll None does not scroll")]
        [Test]
        [Timeout(30_000)]
        public async Task ScrollNoneShouldNotScrollThePage()
        {
            ...
            await page.TypeAsync("#btn", "wave428", force: true, scroll: ActionScroll.None).ConfigureAwait(false);
            double after = await page.EvaluateAsync<double>("document.getElementById('box').scrollTop").ConfigureAwait(false);
            string value = await page.EvaluateAsync<string>("document.getElementById('btn').value").ConfigureAwait(false);
