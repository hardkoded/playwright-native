...
{
    /// <summary>
    /// Official <c>page.press({ strict })</c>.
    /// </summary>
    [TestFixture]
    public class PressStrictTests : PageTestEx
    {
        [PlaywrightTest("elementhandle-press.spec.ts", "strict true throws when two nodes match")]
        [Test]
        [Timeout(30_000)]
        public async Task StrictTrueShouldThrowWhenTwoNodesMatch()
        {
            ...
            PlaywrightNativeException ex = Assert.CatchAsync<PlaywrightNativeException>(
                () => page.PressAsync("input", "a", strict: true));
        ...
        [PlaywrightTest("elementhandle-press.spec.ts", "strict true accepts a unique selector")]
        [Test]
        [Timeout(30_000)]
        public async Task StrictTrueShouldAcceptAUniqueSelector()
        {
            ...
            await page.PressAsync("#only", "a", strict: true).ConfigureAwait(false);
        ...
        [PlaywrightTest("elementhandle-press.spec.ts", "strict false overrides context StrictSelectors")]
        [Test]
        [Timeout(30_000)]
        public async Task StrictFalseShouldOverrideContextStrictSelectors()
        {
            ...
            await page.PressAsync("input", "a", strict: false).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.getElementById('first').value").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("a"));
        }

        [PlaywrightTest("elementhandle-press.spec.ts", "frame honors strict")]
        [Test]
        [Timeout(30_000)]
        public async Task FrameShouldHonorStrict()
        {
            ...
            PlaywrightNativeException ex = Assert.CatchAsync<PlaywrightNativeException>(
                () => frame.PressAsync("input", "a", strict: true));
