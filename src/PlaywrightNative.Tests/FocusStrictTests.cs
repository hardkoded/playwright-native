...
{
    /// <summary>
    /// Official <c>page.focus({ strict })</c>.
    /// </summary>
    [TestFixture]
    public class FocusStrictTests : PageTestEx
    {
        [PlaywrightTest("page-focus.spec.ts", "strict true throws when two nodes match")]
        [Test]
        [Timeout(30_000)]
        public async Task StrictTrueShouldThrowWhenTwoNodesMatch()
        {
            ...
            PlaywrightNativeException ex = Assert.CatchAsync<PlaywrightNativeException>(
                () => page.FocusAsync("input", new() { Strict = true }));
        ...
        [PlaywrightTest("page-focus.spec.ts", "strict true accepts a unique selector")]
        [Test]
        [Timeout(30_000)]
        public async Task StrictTrueShouldAcceptAUniqueSelector()
        {
            ...
            await page.FocusAsync("#only", new() { Strict = true }).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.activeElement && document.activeElement.id").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("only"));
        }

        [PlaywrightTest("page-focus.spec.ts", "strict false overrides context StrictSelectors")]
        [Test]
        [Timeout(30_000)]
        public async Task StrictFalseShouldOverrideContextStrictSelectors()
        {
            ...
            await page.FocusAsync("input", new() { Strict = false }).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.activeElement && document.activeElement.id").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("first"));
        }

        [PlaywrightTest("page-focus.spec.ts", "frame honors strict")]
        [Test]
        [Timeout(30_000)]
        public async Task FrameShouldHonorStrict()
        {
            ...
            PlaywrightNativeException ex = Assert.CatchAsync<PlaywrightNativeException>(
                () => frame.FocusAsync("input", new() { Strict = true }));
