  ...
  queryAll(root, selector) {
    return Array.from(root.querySelectorAll(selector));
        ...
        public async Task ShouldWork()
        {
            ...
            PlaywrightSharpException error = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("tAG=DIV"));
  ...
  queryAll(root, selector) {
    ...
    queryAll(root, selector) {
      return window['__answer'] ? [window['__answer'], document.body, document.documentElement] : [];
        ...
        public async Task ShouldHandleErrors()
        {
            await using IBrowser browser = await BrowserLauncher.LaunchAsync().ConfigureAwait(false);
            IPage page = await browser.NewPageAsync().ConfigureAwait(false);
            PlaywrightSharpException error = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("neverregister=ignored"));
    ...
    queryAll(root, selector) {
      return Array.from(root.querySelectorAll('dummy'));
            ...
            error = Assert.CatchAsync<PlaywrightSharpException>(() => Playwright.Selectors.RegisterAsync("$", createDummySelector));
            ...
            error = Assert.CatchAsync<PlaywrightSharpException>(() => Playwright.Selectors.RegisterAsync("dummy", createDummySelector));
            Assert.That(error.Message, Is.EqualTo("selectors.register: \"dummy\" selector engine has been already registered"));

            error = Assert.CatchAsync<PlaywrightSharpException>(() => Playwright.Selectors.RegisterAsync("css", createDummySelector));
    ...
    queryAll(root, selector) {
      return [[document.body]];
            ...
            await using IBrowser browser = await BrowserLauncher.LaunchAsync().ConfigureAwait(false);
            IPage page = await browser.NewPageAsync().ConfigureAwait(false);
            PlaywrightSharpException error = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("__fake=value2"));
