...
namespace PlaywrightSharp.Tests
{
    ...
    public class SelectorsFrameParityTests : PageTestEx
    {
        ...
        public async Task DollarEvalShouldThrowForMissingFrame()
        {
            ...
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.GoToAsync(EmptyPage).ConfigureAwait(false);
            PlaywrightSharpException pageError = Assert.CatchAsync<PlaywrightSharpException>(() => page.EvalOnSelectorAsync<int>("iframe >> internal:control=enter-frame >> canvas", "e => 1"));
            Assert.That(pageError.Message, Does.Contain("page.$eval: Failed to find element matching selector"));
            IElementHandle body = await page.QuerySelectorAsync("body").ConfigureAwait(false);
            PlaywrightSharpException handleError = Assert.CatchAsync<PlaywrightSharpException>(() => body.EvalOnSelectorAsync<int>("iframe >> internal:control=enter-frame >> canvas", "e => 1"));
        ...
        public async Task ShouldNotAllowDanglingEnterFrame()
        {
            ...
            PlaywrightSharpException error = Assert.CatchAsync<PlaywrightSharpException>(() => button.ClickAsync());
        ...
        public async Task ShouldNotAllowLeadingEnterFrame()
        {
            ...
            await RouteIframeAsync(page).ConfigureAwait(false);
            await page.GoToAsync(EmptyPage).ConfigureAwait(false);
            PlaywrightSharpException error = Assert.CatchAsync<PlaywrightSharpException>(() => page.WaitForSelectorAsync("internal:control=enter-frame >> button"));
    ...
    document.body.appendChild(iframe);
        ...
        public async Task ShouldNonWorkForNonFrame()
        {
            ...
            PlaywrightSharpException error = Assert.CatchAsync<PlaywrightSharpException>(() => button.WaitForAsync());
        ...
        public async Task ShouldThrowWhenPiercingFramesMatchesMultipleFrames()
        {
            ...
            PlaywrightSharpException error = Assert.CatchAsync<PlaywrightSharpException>(() => page.Locator("internal:control=pierce-frames >> div").InnerHTMLAsync());
        ...
        public async Task ShouldNotAllowPierceFramesInTheMiddleOfASelector()
        {
            ...
            await RouteIframeAsync(page).ConfigureAwait(false);
            await page.GoToAsync(EmptyPage).ConfigureAwait(false);
            PlaywrightSharpException error = Assert.CatchAsync<PlaywrightSharpException>(() => page.Locator("iframe >> internal:control=pierce-frames >> div").WaitForAsync());
        ...
        public async Task ShouldNotAllowPierceFramesAfterEnteringAFrame()
        {
            ...
            await using IBrowserContext context = await browser.NewContextAsync().ConfigureAwait(false);
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            PlaywrightSharpException error = Assert.CatchAsync<PlaywrightSharpException>(() => page.Locator("iframe >> internal:control=enter-frame >> internal:control=pierce-frames >> button").CountAsync());
            Assert.That(error.Message, Does.Contain("\"pierce-frames\" is only allowed as the first selector token"));
        }
        ...
        public async Task ShouldNotAllowDanglingEnterFrameWhilePiercing()
        {
            ...
            await using IBrowserContext context = await browser.NewContextAsync().ConfigureAwait(false);
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            PlaywrightSharpException error = Assert.CatchAsync<PlaywrightSharpException>(() => page.Locator("internal:control=pierce-frames >> iframe >> internal:control=enter-frame").CountAsync());
