...
{
    /// <summary>
    /// Official persistent-context <c>baseURL</c> launch option.
    /// </summary>
    [TestFixture]
    public class LaunchPersistentBaseUrlTests : PageTestEx
    {
        private static SimpleServer Server => TestServerSetup.Server;

        [PlaywrightTest("defaultbrowsercontext-1.spec.ts", "LaunchPersistentContextAsync BaseURL")]
        [Test]
        [Timeout(60_000)]
        public async Task LaunchPersistentContextAsyncShouldHonorBaseUrl()
        {
            if (Server == null)
            {
            ...
            string userDataDir = Path.Combine(Path.GetTempPath(), "pwsharp-persist-baseurl-" + Guid.NewGuid().ToString("N"));
            ...
            try
            {
                IBrowserContext context = await browserType.LaunchPersistentContextAsync(userDataDir, new BrowserTypeLaunchPersistentContextOptions
                {
                    ...
                    Headless = true,
                    BaseURL = TestConstants.ServerUrl,
