...
{
    /// <summary>
    /// Official <c>page.tap({ strict })</c>.
    /// </summary>
    [TestFixture]
    public class TapStrictTests : PageTestEx
    {
        [PlaywrightTest("tap.spec.ts", "strict true throws when two nodes match")]
        [Test]
        [Timeout(30_000)]
        public async Task StrictTrueShouldThrowWhenTwoNodesMatch()
        {
            ...
            PlaywrightSharpException ex = Assert.CatchAsync<PlaywrightSharpException>(
                () => page.TapAsync("button", new() { Strict = true }));
        ...
        [PlaywrightTest("tap.spec.ts", "strict true accepts a unique selector")]
        [Test]
        [Timeout(30_000)]
        public async Task StrictTrueShouldAcceptAUniqueSelector()
        {
            ...
            await using IBrowserContext context = await browser.NewContextAsync(new() { HasTouch = true }).ConfigureAwait(false);
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<button id='only' onclick='this.dataset.hit=1'>one</button><button>two</button>").ConfigureAwait(false);

            await page.TapAsync("#only", new() { Strict = true }).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.getElementById('only').dataset.hit").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("1"));
        }

        [PlaywrightTest("tap.spec.ts", "strict false overrides context StrictSelectors")]
        [Test]
        [Timeout(30_000)]
        public async Task StrictFalseShouldOverrideContextStrictSelectors()
        {
            ...
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<button id='first' onclick='this.dataset.hit=1'>one</button><button>two</button>").ConfigureAwait(false);

            await page.TapAsync("button", new() { Strict = false }).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.getElementById('first').dataset.hit").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("1"));
        }

        [PlaywrightTest("tap.spec.ts", "frame honors strict")]
        [Test]
        [Timeout(30_000)]
        public async Task FrameShouldHonorStrict()
        {
            ...
            PlaywrightSharpException ex = Assert.CatchAsync<PlaywrightSharpException>(
                () => frame.TapAsync("button", new() { Strict = true }));
