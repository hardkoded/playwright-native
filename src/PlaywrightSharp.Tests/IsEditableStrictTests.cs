...
{
    /// <summary>
    /// Official <c>page.isEditable({ strict })</c>.
    /// </summary>
    [TestFixture]
    public class IsEditableStrictTests : PageTestEx
    {
        ...
        public async Task StrictTrueShouldThrowWhenTwoNodesMatch()
        {
            ...
            PlaywrightSharpException ex = Assert.CatchAsync<PlaywrightSharpException>(
                () => page.IsEditableAsync("input", new() { Strict = true }));
        ...
        public async Task StrictTrueShouldAcceptAUniqueSelector()
        {
            ...
            await page.IsEditableAsync("#only", new() { Strict = true }).ConfigureAwait(false);
            bool actual = await page.EvaluateAsync<bool>("!document.getElementById('only').readOnly").ConfigureAwait(false);
        ...
        public async Task StrictFalseShouldOverrideContextStrictSelectors()
        {
            ...
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<input id='first'><input>").ConfigureAwait(false);

            await page.IsEditableAsync("input", new() { Strict = false }).ConfigureAwait(false);
            bool actual = await page.EvaluateAsync<bool>("!document.getElementById('first').readOnly").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo(true));
        }
        ...
        public async Task FrameShouldHonorStrict()
        {
            ...
            PlaywrightSharpException ex = Assert.CatchAsync<PlaywrightSharpException>(
                () => frame.IsEditableAsync("input", new() { Strict = true }));
