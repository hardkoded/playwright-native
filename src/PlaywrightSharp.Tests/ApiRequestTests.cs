...
namespace PlaywrightSharp.Tests
{
    ...
    public class ApiRequestTests : PageTestEx
    {
        ...
        public async Task GetShouldThrowWhenFailOnStatusCode()
        {
            ...
            PlaywrightSharpException ex = Assert.ThrowsAsync<PlaywrightSharpException>(async () =>
            {
        ...
        public async Task GetShouldNotRetryWhenMaxRetriesIsZero()
        {
            ...
            PlaywrightSharpException resetError = Assert.ThrowsAsync<PlaywrightSharpException>(async () =>
            {
        ...
        public async Task StandaloneUserAgentShouldBeSent()
        {
            ...
            await using IAPIRequestContext request = await Playwright.APIRequest.NewContextAsync(new() { UserAgent = "PlaywrightSharp-Wave-147" }).ConfigureAwait(false);
            IAPIResponse response = await request.GetAsync(
                TestConstants.ServerUrl + "/api-ua").ConfigureAwait(false);
            Assert.That(await response.TextAsync().ConfigureAwait(false), Is.EqualTo("PlaywrightSharp-Wave-147"));
        }
