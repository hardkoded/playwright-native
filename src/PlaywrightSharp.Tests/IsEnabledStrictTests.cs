...
{
    /// <summary>
    /// Official <c>page.isEnabled({ strict })</c>.
    /// </summary>
    [TestFixture]
    public class IsEnabledStrictTests : PageTestEx
    {
        ...
        public async Task StrictTrueShouldThrowWhenTwoNodesMatch()
        {
            ...
            PlaywrightSharpException ex = Assert.CatchAsync<PlaywrightSharpException>(
                () => page.IsEnabledAsync("button", new() { Strict = true }));
        ...
        public async Task StrictTrueShouldAcceptAUniqueSelector()
        {
            ...
            await page.IsEnabledAsync("#only", new() { Strict = true }).ConfigureAwait(false);
        ...
        public async Task StrictFalseShouldOverrideContextStrictSelectors()
        {
            ...
            await page.IsEnabledAsync("button", new() { Strict = false }).ConfigureAwait(false);
        ...
        public async Task FrameShouldHonorStrict()
        {
            ...
            PlaywrightSharpException ex = Assert.CatchAsync<PlaywrightSharpException>(
                () => frame.IsEnabledAsync("button", new() { Strict = true }));
