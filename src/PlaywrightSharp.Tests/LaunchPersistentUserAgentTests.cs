...
{
    /// <summary>
    /// Official persistent-context <c>userAgent</c> launch option.
    /// </summary>
    [TestFixture]
    public class LaunchPersistentUserAgentTests : PageTestEx
    {
        [PlaywrightTest("defaultbrowsercontext-1.spec.ts", "LaunchPersistentContextAsync UserAgent")]
        [Test]
        [Timeout(60_000)]
        public async Task LaunchPersistentContextAsyncShouldHonorUserAgent()
        {
            ...
            string userDataDir = Path.Combine(Path.GetTempPath(), "pwsharp-persist-ua-" + Guid.NewGuid().ToString("N"));
