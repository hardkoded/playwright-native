...
namespace PlaywrightSharp.Tests
{
  ...
  ").ConfigureAwait(false);
        ...
        public async Task PressShouldThrowOnUnknownKeys()
        {
            ...
            PlaywrightSharpException unknown = Assert.CatchAsync<PlaywrightSharpException>(() => locator.PressAsync("NotARealKey"));
            Assert.That(unknown, Is.Not.Null);
            Assert.That(unknown.Message, Does.Match(new Regex("Unknown key: \"NotARealKey\"")));
            PlaywrightSharpException yo = Assert.CatchAsync<PlaywrightSharpException>(() => locator.PressAsync("ё"));
            Assert.That(yo, Is.Not.Null);
            Assert.That(yo.Message, Does.Match(new Regex("Unknown key: \"ё\"")));
            PlaywrightSharpException emoji = Assert.CatchAsync<PlaywrightSharpException>(() => locator.PressAsync("😊"));
