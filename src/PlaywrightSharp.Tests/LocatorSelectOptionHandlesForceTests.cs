...
{
    /// <summary>
    /// Official <c>locator.selectOption(IEnumerable&lt;IElementHandle&gt;, { force })</c>.
    /// </summary>
    [TestFixture]
    public class LocatorSelectOptionHandlesForceTests : PageTestEx
    {
        ...
        public async Task ForceTrueShouldSelectAHiddenSelect()
        {
            ...
            await page.SetContentAsync("<select id='only' style='display:none'><option value='wave688'>a</option></select>").ConfigureAwait(false);
            ...
            Assert.That(actual, Is.EqualTo("wave688"));
        }
        ...
        public async Task ForceTrueShouldAcceptAVisibleUniqueSelector()
        {
            ...
            await using IBrowserContext context = await browser.NewContextAsync().ConfigureAwait(false);
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<select id='only'><option value='wave688'>a</option></select>").ConfigureAwait(false);
            IElementHandle option = await page.QuerySelectorAsync("#only option").ConfigureAwait(false);
            List<IElementHandle> values = new List<IElementHandle> { option };
        ...
        public async Task FrameLocatorShouldHonorForce()
        {
            ...
            await frame.SetContentAsync("<select id='only' style='display:none'><option value='wave688'>a</option></select>").ConfigureAwait(false);
