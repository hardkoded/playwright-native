...
namespace PlaywrightSharp.Tests
{
    ...
    public class InterceptionParityTests : PageTestEx
    {
        ...
        {
            Exception unmatchedOpen = Assert.Throws<PlaywrightSharpException>(() => UrlMatcher.GlobToRegexPattern("{foo"));
            Assert.That(unmatchedOpen.Message, Does.Contain("Invalid glob pattern \"{foo\": unmatched '{'"));
            Exception unmatchedClose = Assert.Throws<PlaywrightSharpException>(() => UrlMatcher.GlobToRegexPattern("}foo"));
            Assert.That(unmatchedClose.Message, Does.Contain("Invalid glob pattern \"}foo\": unmatched '}'"));
            Assert.That(
                Assert.Throws<PlaywrightSharpException>(() => UrlMatcher.GlobToRegexPattern("http://*/foo{")).Message,
                Does.Contain("unmatched '{'"));
            Assert.That(
                Assert.Throws<PlaywrightSharpException>(() => UrlMatcher.GlobToRegexPattern("**/*.png?{")).Message,
                Does.Contain("unmatched '{'"));
            Assert.That(
                Assert.Throws<PlaywrightSharpException>(() => UrlMatcher.GlobToRegexPattern("https://example.com/{a")).Message,
                Does.Contain("unmatched '{'"));
            Assert.That(
                Assert.Throws<PlaywrightSharpException>(() => UrlMatcher.GlobToRegexPattern("{{foo}")).Message,
