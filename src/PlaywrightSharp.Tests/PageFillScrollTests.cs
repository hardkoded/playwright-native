...
{
    /// <summary>
    /// Official <c>scroll</c> option on <see cref="IPage.FillAsync"/>.
    /// </summary>
    [TestFixture]
    public class PageFillScrollTests : PageTestEx
    {
        [PlaywrightTest("page-fill.spec.ts", "Fill Auto scrolls the target into view")]
        [Test]
        [Timeout(30_000)]
        public async Task AutoShouldScrollTheTargetIntoView()
        {
            ...
            await page.FillAsync("#btn", "wave414").ConfigureAwait(false);
            ...
            Assert.That(value, Is.EqualTo("wave414"));
        }

        [PlaywrightTest("page-fill.spec.ts", "Fill scroll None does not scroll")]
        [Test]
        [Timeout(30_000)]
        public async Task ScrollNoneShouldNotScrollThePage()
        {
            ...
            await page.FillAsync("#btn", "wave414", force: true, scroll: ActionScroll.None).ConfigureAwait(false);
            double after = await page.EvaluateAsync<double>("document.getElementById('box').scrollTop").ConfigureAwait(false);
            string value = await page.EvaluateAsync<string>("document.getElementById('btn').value").ConfigureAwait(false);
