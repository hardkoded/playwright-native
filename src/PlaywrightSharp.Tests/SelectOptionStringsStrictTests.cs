...
{
    /// <summary>
    /// Official <c>page.selectOption(IEnumerable&lt;string&gt;, { strict })</c>.
    /// </summary>
    [TestFixture]
    public class SelectOptionStringsStrictTests : PageTestEx
    {
        private static IEnumerable<string> WaveValues()
            => new List<string> { "wave673" };
        ...
        public async Task StrictTrueShouldThrowWhenTwoSelectsMatch()
        {
            ...
            await page.SetContentAsync("<select><option value='wave673'>a</option></select><select><option value='wave673'>b</option></select>").ConfigureAwait(false);

            PlaywrightSharpException ex = Assert.CatchAsync<PlaywrightSharpException>(
                () => page.SelectOptionAsync("select", WaveValues(), strict: true));
        ...
        public async Task StrictTrueShouldAcceptAUniqueSelector()
        {
            ...
            await using IBrowserContext context = await browser.NewContextAsync().ConfigureAwait(false);
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<select id='only'><option value='wave673'>a</option></select><select><option value='wave673'>b</option></select>").ConfigureAwait(false);

            await page.SelectOptionAsync("#only", WaveValues(), strict: true).ConfigureAwait(false);
            string actual = await page.EvaluateAsync<string>("document.getElementById('only').value").ConfigureAwait(false);
            Assert.That(actual, Is.EqualTo("wave673"));
        }
        ...
        public async Task StrictFalseShouldOverrideContextStrictSelectors()
        {
            ...
            IPage page = await context.NewPageAsync().ConfigureAwait(false);
            await page.SetContentAsync("<select id='first'><option value='wave673'>a</option></select><select><option value='wave673'>b</option></select>").ConfigureAwait(false);

            await page.SelectOptionAsync("select", WaveValues(), strict: false).ConfigureAwait(false);
        ...
        public async Task FrameShouldHonorStrict()
        {
            ...
            await frame.SetContentAsync("<select><option value='wave673'>a</option></select><select><option value='wave673'>b</option></select>").ConfigureAwait(false);

            PlaywrightSharpException ex = Assert.CatchAsync<PlaywrightSharpException>(
                () => frame.SelectOptionAsync("select", WaveValues(), strict: true));
