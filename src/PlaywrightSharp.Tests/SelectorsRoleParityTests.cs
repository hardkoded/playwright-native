...
namespace PlaywrightSharp.Tests
{
  ...
  ").ConfigureAwait(false);
        ...
        public async Task Errors()
        {
            ...
            PlaywrightSharpException e0 = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("role=[bar]"));
            Assert.That(e0.Message, Does.Contain("Role must not be empty"));

            PlaywrightSharpException e1 = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("role=foo[sElected]"));
            ...
            PlaywrightSharpException e2 = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("role=foo[bar . qux=true]"));
            Assert.That(e2.Message, Does.Contain("Unknown attribute \"bar.qux\""));

            PlaywrightSharpException e3 = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("role=heading[level=\"bar\"]"));
            Assert.That(e3.Message, Does.Contain("\"level\" attribute must be compared to a number"));

            PlaywrightSharpException e4 = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("role=checkbox[checked=\"bar\"]"));
            Assert.That(e4.Message, Does.Contain("\"checked\" must be one of true, false, \"mixed\""));

            PlaywrightSharpException e5 = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("role=checkbox[checked~=true]"));
            Assert.That(e5.Message, Does.Contain("cannot use ~= in attribute with non-string matching value"));

            PlaywrightSharpException e6 = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("role=button[level=3]"));
            Assert.That(e6.Message, Does.Contain("\"level\" attribute is only supported for roles: \"heading\", \"listitem\", \"row\", \"treeitem\""));

            PlaywrightSharpException e7 = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("role=button[name]"));
            Assert.That(e7.Message, Does.Contain("\"name\" attribute must have a value"));

            PlaywrightSharpException e8 = Assert.CatchAsync<PlaywrightSharpException>(() => page.QuerySelectorAsync("role=treeitem[expanded=\"none\"]"));
