# Official APIResponse leftover campaign

The web storage table ended at Wave 622. Official Playwright still
has `APIResponse.serverAddr()` this repo lacked.
...
**Previous campaign:** `tasks/web-storage-campaign.md`
**Next campaign:** `tasks/official-leftover-campaign.md`

## Goal

Port official playwright-dotnet `IAPIResponse` network metadata that
this repo still lacks:

- `serverAddr()` — remote IP and port
- `securityDetails()` — TLS info, null for HTTP
- `Timing` — resource timing (unknown fields are `-1`)

Do **not** invent inspector-only APIs (`pickLocator`). Wire the
HttpClient `APIRequest` path (same for CR + WK).
...
| Wave | Slice |
|------|--------|
| **623** | `IAPIResponse.ServerAddrAsync` — done |
| **624** | `IAPIResponse.SecurityDetailsAsync` — done |
| **625** | `IAPIResponse.Timing` — done |
