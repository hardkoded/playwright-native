# Official page web storage leftover campaign

The dblclick steps table ended at Wave 620. Official Playwright still
has `page.localStorage` / `page.sessionStorage` this repo lacked.
...
**Previous campaign:** `tasks/dblclick-steps-campaign.md`
**Next campaign:** `tasks/api-response-leftover-campaign.md`

## Goal

Port official Playwright `IWebStorage` (playwright-dotnet / Node) that
this repo still lacks:

- `page.localStorage` — origin-scoped get/set/items/remove/clear
- `page.sessionStorage` — same API on `sessionStorage`

Do **not** invent inspector-only APIs (`pickLocator`). Wire CR + WK.
...
| Wave | Slice |
|------|--------|
| **621** | `IPage.LocalStorage` — done |
| **622** | `IPage.SessionStorage` — done |
