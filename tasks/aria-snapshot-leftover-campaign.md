# Official aria snapshot leftover campaign

The locator filter table ended at Wave 608. Official Playwright still has
aria snapshot APIs this repo lacks.
...
**Previous campaign:** `tasks/locator-filter-campaign.md`
**Next campaign:** `tasks/frame-locator-leftover-campaign.md`

## Goal

Port official Playwright aria snapshot APIs that exist on Node and
`microsoft/playwright-dotnet` and are still missing here:

- `page.ariaSnapshot()` (no selector; equivalent to `locator('body')`)
- `locator.ariaSnapshot({ depth })`
- `locator.ariaSnapshot({ boxes })`
...
| Wave | Slice |
|------|--------|
| **609** | `IPage.AriaSnapshotAsync()` (no selector) — done |
| **610** | `AriaSnapshotAsync` `depth` — done |
| **611** | `AriaSnapshotAsync` `boxes` — done |
