# Official locator filter leftover campaign

The v1.60 leftover table ended at Wave 604. Official Playwright still has
locator filter / describe / snapshot options this repo lacks.
...
**Previous campaign:** `tasks/v160-leftover-campaign.md`
**Next campaign:** `tasks/aria-snapshot-leftover-campaign.md`

## Goal

Port official Playwright locator options that exist on Node and
`microsoft/playwright-dotnet` and are still missing here:

- `locator.filter({ visible })`
- `locator.description` (getter)
- `locator.ariaSnapshot({ mode })`
- `page.locator(otherLocator)`
...
| Wave | Slice |
|------|--------|
| **605** | `ILocator.Filter(bool visible)` — done |
| **606** | `ILocator.Description` — done |
| **607** | `AriaSnapshotAsync` `mode` — done |
| **608** | `IPage.Locator(ILocator)` — done |
